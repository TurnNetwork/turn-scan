package com.turn.browser.proxyppos;
...
public class ProxyContract extends Contract {
    ...
    public Observable<ProxyEventEventResponse> proxyEventEventObservable(BubbleFilter filter) {
        return web3j.bubbleLogObservable(filter).map(new Func1<Log, ProxyEventEventResponse>() {
            @Override
