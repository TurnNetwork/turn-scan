...
/**
 * Unified configuration items for chain parameters
 *
 * @Description:
 */
@Slf4j
...
public class RetryableClient {
    ...
    /**
     * 0 block waiting cycle access time
     */
    ...
    // Delegate reward contract interface
        ...
        try {
            ...
            addresses.forEach(address -> {
                ...
                if (protocol == Web3jProtocolEnum.WS) {
                    ...
                    log.error("Web3j connection protocol [{}] is illegal!", protocol.getHead());
                    System.exit(1);
                }
            ...
            if (web3jWrappers.isEmpty()) throw new ConfigLoadingException("No Web3j instance available!");
    ...
    public void updateCurrentWeb3jWrapper() {
        ...
        try {
            ...
            if (preWeb3j == null || preWeb3j != currentWeb3jWrapper) {
                // If the predecessor web3j is empty or Web3j has changed, the contract variables will be updated.
                updateContract();
            }
            if (maxBlockNumber == -1) {
    ...
    public void zeroBlockNumberWait() {
        try {
            while (true) {
                BigInteger nowBlockNumber = getWeb3jWrapper().getWeb3j().bubbleBlockNumber().send().getBlockNumber();
                if (nowBlockNumber.compareTo(zeroBlockNumber) > 0) {
                    log.debug("Block generation started, current block height is {}", nowBlockNumber.longValue());
                    break;
                }
                Thread.sleep(1000L * zeroBlockNumberWaitTime);
                log.error("Waiting for block...");
            }
        } catch (Exception e) {
