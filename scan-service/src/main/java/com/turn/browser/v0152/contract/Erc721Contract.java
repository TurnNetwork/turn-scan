...
import com.bubble.abi.solidity.datatypes.generated.Bytes4;
...
public class Erc721Contract extends Contract implements ErcContract {
    ...
    public Observable<OwnershipTransferredEventResponse> ownershipTransferredEventObservable(BubbleFilter filter) {
        return web3j.bubbleLogObservable(filter).map(log -> {
    ...
    public RemoteCall<TransactionReceipt> safeTransferFrom(String _from, String _to, BigInteger _tokenId, byte[] _data) {
        final Function function = new Function(FUNC_SAFETRANSFERFROM, Arrays.asList(new Address(_from), new Address(_to), new Uint256(_tokenId), new com.bubble.abi.solidity.datatypes.DynamicBytes(_data)), Collections.emptyList());
        return executeRemoteCallTransaction(function);
    }
